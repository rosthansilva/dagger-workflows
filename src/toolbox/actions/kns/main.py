...
class Kns:
    ...
    async def kubectl(
        self, 
        command: Annotated[str, Doc("O comando kubectl a ser executado. Ex: 'get pods -A'")]
    ) -> str:
        """
        ...
        return await client.run_kubectl(
    ...
    async def test_internal_connection(self) -> str:
        ...
        result = await k8s_client.run_kubectl(
            kube_config=my_config,
            command="get nodes -o wide",
            cluster_service=my_service
        )
