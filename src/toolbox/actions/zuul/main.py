...
"""
        ...
        return source.with_new_file(f"zuul.d/jobs-{name}.yaml", job_yaml).with_new_file(
            f"playbooks/{name}/run.yaml", playbook_boilerplate
        )
            ...
            await (
                ...
                .with_exec(
                    [
                        "python3",
                        "-c",
                        "import yaml, glob; [yaml.safe_load(open(f)) for f in glob.glob('zuul.d/*.yaml')]",
                    ]
                )
